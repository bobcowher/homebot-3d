...
Creates a HIDDEN GLFW window, runs the one-step-per-frame loop for a short spell
(including a forced reset to rebuild scene/context), and renders each frame.
Exits 0 on success. Requires a GL-capable display/driver; if GLFW cannot init
here that's environmental, not a code fault.
"""
import glfw
import mujoco
...
for frame in range(30):
    ...
    env.step_physics(action)                           # one step per frame
